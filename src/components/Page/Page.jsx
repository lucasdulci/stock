...
export const Page = () => {
  ...
  const eliminarProducto = (id) => {
    setControlador((prevControlador) =>
        prevControlador.filter((producto) => producto.id !== id)
    );
};
  ...
  const getStockColorClass = (stock, minimoStock) => {
    return stock >= parseInt(minimoStock, 10) ? "bg-green-500" : "bg-red-500";
};
